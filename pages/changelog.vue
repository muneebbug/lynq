...
<template>
  <div class="container py-8 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="space-y-16">
      <div v-for="(release, index) in changelog" :key="index" class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <div class="flex items-center mt-2 text-muted-foreground">
              <Badge variant="outline" class="mr-2">{{ release.version }}</Badge>
        ...
        <p v-if="release.description" class="text-muted-foreground">
          {{ release.description }}
        </p>

        <div class="space-y-6 pl-4 border-l-2 border-muted">
          <div v-for="(change, changeIndex) in release.changes" :key="changeIndex" 
               class="relative transition-all">
            <!-- Dot indicator -->
            <div class="absolute -left-[25px] mt-1.5 h-4 w-4 rounded-full border-2 border-background bg-primary"></div>
            
            <div class="mb-2 flex items-center gap-2">
              <component :is="getChangeTypeIcon(change.type)" class="h-5 w-5" />
            ...
            <p v-if="change.description" class="text-muted-foreground ml-7">
              {{ change.description }}
            </p>
          </div>
        </div>

        <Separator v-if="index < changelog.length - 1" class="mt-8" />
      </div>
    </div>
  </div>
</template> 